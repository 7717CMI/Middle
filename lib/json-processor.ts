...
export async function processJsonDataAsync(
  ...
  try {
    ...
    if (volumeData) {
      ...
      console.log('🔊 Volume data geographies:', Object.keys(volumeData))
      console.log('🔊 Volume data segment types for first geography:',
        volumeData[Object.keys(volumeData)[0]] ? Object.keys(volumeData[Object.keys(volumeData)[0]]) : 'NONE')

      for (const segmentType of segmentTypes) {
        console.log(`🔊 Processing volume for segment type: ${segmentType}`)
        ...
        console.log(`🔊 Volume records for ${segmentType}: ${volumeRecs.length}`)
        volumeRecords.push(...volumeRecs)
      }
      console.log(`🔊 Total volume records created: ${volumeRecords.length}`)
      if (volumeRecords.length > 0) {
        console.log('🔊 Sample volume record:', {
          geo: volumeRecords[0].geography,
          segment: volumeRecords[0].segment,
          segType: volumeRecords[0].segment_type,
          timeSeries: Object.entries(volumeRecords[0].time_series).slice(0, 3)
        })
      }
    } else {
      console.log('🔊 Volume data is NULL - no volume records will be created')
    }
...
export async function loadAndProcessJsonFiles(
  ...
  try {
    ...
    if (volumeContent) {
      console.log(`Volume JSON size: ${(volumeContent.length / 1024 / 1024).toFixed(2)} MB`)
    }
