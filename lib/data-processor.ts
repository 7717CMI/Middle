...
export function filterData(
  ...
  if (effectiveAggregationLevel === null || effectiveAggregationLevel === undefined) {
    ...
    // IMPORTANT: In geography-mode, we aggregate by geography, not by segment level
    // So we should NOT enforce Level 2 segment aggregation in geography-mode
    if (filters.viewMode === 'geography-mode') {
      // In geography-mode, show all records (leaf level) and aggregate by geography in chart prep
...
export function prepareGroupedBarData(
  ...
  // IMPORTANT: Only apply Level 2 segment aggregation in segment-mode, NOT in geography-mode
  const isLevel2 = effectiveAggregationLevel === 2 && viewMode === 'segment-mode'
...
export function prepareLineChartData(
  ...
  // Only apply Level 2 aggregation when user hasn't explicitly selected segments AND in segment-mode
  const isLevel2 = effectiveAggregationLevel === 2 && !hasUserSelectedSegments && viewMode === 'segment-mode'
  ...
  return years.map(year => {
    ...
    records.forEach(record => {
      ...
      if (viewMode === 'segment-mode') {
        ...
        if (shouldAggregateBySelectedSegment && !isLevel2) {
          const hierarchy = record.segment_hierarchy
...
export function prepareWaterfallData(
  ...
  // Only apply Level 2 aggregation when user hasn't explicitly selected segments AND in segment-mode
  const isLevel2 = effectiveAggregationLevel === 2 && !hasUserSelectedSegments && filters.viewMode === 'segment-mode'
