...
export function GroupedBarChart({ title, height = 400 }: GroupedBarChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    // Get the appropriate dataset with safeguard for undefined volume
    const dataset = filters.dataType === 'value'
      ? data.data.value.geography_segment_matrix
      : (data.data.volume?.geography_segment_matrix || [])

    // Debug: Log dataset info for volume
    console.log('📊 Dataset Selection Debug:', {
      dataType: filters.dataType,
      valueRecordsCount: data.data.value?.geography_segment_matrix?.length || 0,
      volumeRecordsCount: data.data.volume?.geography_segment_matrix?.length || 0,
      selectedDatasetCount: dataset?.length || 0,
      hasVolume: data.metadata?.has_volume,
      sampleVolumeRecord: data.data.volume?.geography_segment_matrix?.[0] ? {
        geo: data.data.volume?.geography_segment_matrix?.[0]?.geography,
        seg: data.data.volume?.geography_segment_matrix?.[0]?.segment,
        ts: Object.entries(data.data.volume?.geography_segment_matrix?.[0]?.time_series || {}).slice(0, 3)
      } : 'NO VOLUME RECORDS'
    })

    // If volume is selected but no volume data exists, show warning
    if (filters.dataType === 'volume' && dataset.length === 0) {
      console.warn('⚠️ Volume data type selected but no volume records available!')
      console.warn('Make sure volume.json was uploaded with the same structure as value.json')
    }
    ...
    // 🔊 Debug: Log filtering results for volume
    if (filters.dataType === 'volume') {
      console.log('🔊 Volume Filtering Debug:', {
        datasetLength: dataset.length,
        filteredLength: filtered.length,
        filterGeographies: filters.geographies,
        filterSegmentType: filters.segmentType,
        filterAggregationLevel: filters.aggregationLevel,
        effectiveAggregationLevel,
        datasetGeographies: [...new Set(dataset.map(r => r.geography))],
        datasetSegmentTypes: [...new Set(dataset.map(r => r.segment_type))],
        filteredRecords: filtered.slice(0, 5).map(r => ({
          geo: r.geography,
          seg: r.segment,
          segType: r.segment_type,
          level: r.aggregation_level,
          isAgg: r.is_aggregated,
          ts: Object.entries(r.time_series).slice(0, 2)
        }))
      })

      // If dataset has records but filtered is empty, log why
      if (dataset.length > 0 && filtered.length === 0) {
        console.warn('🔊 Volume data was filtered to ZERO records!')
        const sampleRecord = dataset[0]
        console.warn('🔊 Sample volume record:', {
          geo: sampleRecord.geography,
          seg: sampleRecord.segment,
          segType: sampleRecord.segment_type,
          level: sampleRecord.aggregation_level
        })
        console.warn('🔊 Filter values:', {
          geoFilter: filters.geographies,
          segTypeFilter: filters.segmentType,
          levelFilter: effectiveAggregationLevel
        })
        console.warn('🔊 Would pass geography filter?',
          filters.geographies.length === 0 || filters.geographies.includes(sampleRecord.geography))
        console.warn('🔊 Would pass segmentType filter?',
          sampleRecord.segment_type === filters.segmentType)
      }
    }
    ...
    console.log('📊 After filtering:', {
      ...
      sampleFiltered: filtered.slice(0, 5).map(r => ({
        ...
        timeSeries: Object.entries(r.time_series || {}).slice(0, 3),
        hasData: Object.values(r.time_series || {}).some(v => v > 0)
      }))
    })

    // CRITICAL: Check if all filtered records have empty time_series
    const recordsWithData = filtered.filter(r =>
      Object.values(r.time_series || {}).some(v => typeof v === 'number' && v > 0)
    )
    console.log('📊 Records with non-zero time_series:', {
      total: filtered.length,
      withData: recordsWithData.length,
      noData: filtered.length - recordsWithData.length
    })
