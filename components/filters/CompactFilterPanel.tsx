...
export function CompactFilterPanel() {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-3 space-y-2.5">
      {/* Data Type - Value only */}
      <div>
        <label className="text-xs font-medium text-black">Data Type</label>
        <div className="flex gap-1 mt-1">
          <button
            className="flex-1 px-2 py-1 text-xs rounded bg-blue-600 text-white"
          >
            Value
          </button>
        </div>
      ...
      <div>
        ...
        <div className="flex gap-1 mt-1">
          <button
            onClick={() => {
              const historicalEndYear = data.metadata.historical_years.length > 0 
                ? Math.max(...data.metadata.historical_years)
                : data.metadata.base_year
              updateFilters({ 
                yearRange: [historicalEndYear - 2, historicalEndYear] 
              })
            }}
          ...
          <button
            onClick={() => {
              const historicalEndYear = data.metadata.historical_years.length > 0 
                ...
                : data.metadata.base_year
              updateFilters({ 
                yearRange: [historicalEndYear + 1, data.metadata.forecast_year] 
              })
            }}
